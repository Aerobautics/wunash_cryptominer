...
void sha_256(unsigned char input[], unsigned char output[HASH_SIZE], unsigned int messageLength, unsigned int arraySize);
std::vector<unsigned char> sha_256(std::vector<unsigned char> input);
...
void test_function_002() {
	...
	//fdeab9acf371362bd2658cdc9a29e8f9c757fcf9811603a8c447cd1d915118
...
void test_function_003() {
	...
	messageLength = 64;
	arraySize = sizeof(test_block_64);
	sha_256(test_block_64, test_hash, messageLength, arraySize);
	for (int i = 0; i < HASH_SIZE; i++) {
		std::cout << std::hex << static_cast<int>(test_hash[i]);
	}
	std::cout << std::endl;
...
void sha_256(unsigned char input[], unsigned char output[HASH_SIZE], unsigned int messageLength, unsigned int arraySize) {
	...
	//TODO: CHECK ARRAY SIZE TO MAKE SURE CAN ACCOMODATE ADDITIONS
	...
	appendageLength = 0;
	//messageLength = input.size() * WORD_SIZE;
	messageLength = messageLength * WORD_SIZE;
	do {
		totalLength = messageLength + 1 + appendageLength + END_SIZE;
		if (!(totalLength % BIT_COUNT)) {
			isMatchFound = true;
	...
	for (int i = 0; i < additions; i++) {
		if (i == 0) {
			input[messageLength / WORD_SIZE + i] = FIRST_WORD;
		} else {
			input[messageLength / WORD_SIZE + i] = 0;
		}
		counter = messageLength / WORD_SIZE + i;
	}
	tail = decimateInteger(messageLength, END_SIZE / WORD_SIZE);
	//input.insert(input.end(), tail.begin(), tail.end());
	for (int i = 0; i < tail.size(); i++) {
		input[counter + i] = tail[i];
	}
	counter = counter + tail.size();
	// Break the message into 512 bit chunks
	iterations = counter / (BIT_COUNT / WORD_SIZE);
	for (int i = 0; i < iterations; i++) {
		// Breaking off a 512b chunk . . .
		//temporary = std::vector<unsigned char>(input.begin() + i * (BIT_COUNT / WORD_SIZE),
		//	input.begin() + (i + 1) * (BIT_COUNT / WORD_SIZE));
		temporary = std::vector<unsigned char>();
		for (int j = i * (BIT_COUNT / WORD_SIZE); j < (i + 1) * (BIT_COUNT / WORD_SIZE); j++) {
...
std::vector<unsigned char> sha_256(std::vector<unsigned char> input) {
	...
	for (int i = 0; i < HASH_COUNT; i++) {
		...
		//output[i * BYTE_COUNT + 2] = temporary[2];
		//output[i * BYTE_COUNT + 3] = temporary[3];
		output.push_back(temporary[0]);
		output.push_back(temporary[1]);
		output.push_back(temporary[2]);
		output.push_back(temporary[3]);
	}
